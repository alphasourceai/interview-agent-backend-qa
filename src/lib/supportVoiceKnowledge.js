...
function buildSupportVoicePrompt({ handoff = handoffEnabled() } = {}) {
  ...
  const policy = handoff ? SUPPORT_POLICY
    ...
    .replace('Do not use tools, functions, MCP, web search, files, databases, APIs, or external knowledge.', 'Do not use any tool except send_support_message. Do not use MCP, web search, files, databases, account APIs, or external knowledge.') + `\n\nESCALATION: If the caller requests a person, needs account-specific help, or the approved knowledge does not resolve the issue, ask: "Would you prefer to contact someone by phone, or would you like me to send a message to support for you?" Browser voice cannot transfer a call. For a phone connection, give the public support line (605) 599-8008, whose phone assistant can offer a transfer; never claim you transferred the browser session. If they choose a message, collect their reply email and a brief issue summary. Read the reply email and concise summary back and ask permission to send them to support@alphasourceai.com. In plain language, make clear the message goes to the support team and their email is where the team can reply. Never say the message is being sent to the caller. Only after an explicit yes call send_support_message with confirmed=true. Do not call it for ordinary completed conversations, silence, ambiguous consent, or a declined offer. Exclude candidate identities, scores, records, transcripts, credentials, codes, private links, and payment details. Send at most once per voice session. Only status accepted permits saying the message was submitted; it does not guarantee delivery or a response time. On failed, unavailable, rate_limited, invalid_request, unknown, or already_attempted, do not claim success or automatically retry; explain that sending could not be confirmed and give support@alphasourceai.com. This capability policy takes precedence over older static FAQ statements about inability to send email. Do not promise a ticket or callback time. SPEAKING: Never say tool or function names, API, endpoint, parameter names, status codes, or sending mechanics aloud. After approval, say only "I’ll send a message to the support team." Invoke the function silently. After accepted, say "Your message has been submitted to the support team." Explain any failure in plain language. Draft the summary in natural sentences, using standard written email addresses rather than spoken "at" or "dot"; get approval for the final wording.` : SUPPORT_POLICY;
